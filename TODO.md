...
1. **Long-term storage** - `club_prices.csv` now accumulates real history (every run appends, see "Done" below) rather than being overwritten, which covers the near-term need. Moving to SQLite (pattern already proven in the Detroit Putter Co. retailer scraper) is still the eventual step once the CSV gets unwieldy to query/filter by hand.
2. **API layer** - once storage is queryable, expose it over an API rather than everyone reading CSVs/the DB directly. Not yet scoped (framework, auth, read-only vs. write) - revisit once SQLite lands.
...
Cross-site product matching (normalize names - strip suffixes like "- ON SALE", "2026", "Left Handed" - so the same real-world club can be compared side-by-side across sites) is still open - revisit once there's enough cross-site history to make it worth doing.
...
## Done
- [x] **CSV history + price-drop alerting** (`price_alerts.py`, `test_scrapers.py`) - `club_prices.csv` now appends every run's results instead of being overwritten, so `run_timestamp` gives real history (date+time each row was parsed). A `DEDUP_KEY` of `(site, brand, club_type, variant, name, run_timestamp)` drops exact-duplicate rows within a single run (e.g. a scraping bug returning the same listing twice) before appending - it's not a cross-run dedup, since every row in a run shares the same `run_timestamp` and duplicate history rows across different runs are the point, not a bug. Separately, `price_alerts.log_price_drops()` looks up each new row's most recent prior price by `PRODUCT_KEY` (same as `DEDUP_KEY` minus `run_timestamp`) and logs a `PRICE DROP: ...` line (via the existing logger, no email/Slack per current scope) whenever the new price is lower.
- [x] **Structured logging** (`logging_config.py`) - `test_scrapers.py` logs to console always, and to a timestamped file under `club_price_tracker/logs/` (gitignored) when its `WRITE_LOGS_TO_FILE` flag is set to `True`, so a run can be reviewed later without rerunning it. Per-site scrape failures are caught and logged with a traceback instead of killing the whole run.
